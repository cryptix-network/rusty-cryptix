...
pub(crate) struct Inner {
    ...
    notification_channel: Channel<cryptix_rpc_core::Notification>,
}
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            CryptixRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<CryptixRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<CryptixRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                cryptix_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = CryptixRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
