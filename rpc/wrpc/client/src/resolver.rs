//!
//! Module implementing [`Resolver`] client for obtaining public Cryptix wRPC endpoints.
//!
...
///
/// This client operates against service
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/cryptix/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Cryptix p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Cryptix wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
