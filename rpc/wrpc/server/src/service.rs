...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "127.0.0.1:19301".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### CryptixRpcHandler
///
/// [`CryptixRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct CryptixRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl CryptixRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        CryptixRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for CryptixRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "cryptix" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<CryptixRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(CryptixRpcHandler::new(tasks, *encoding, core_service, options.clone()));
