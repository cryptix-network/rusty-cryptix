// Example of simple client to connect with Cryptix node using wRPC connection and collect some node and network basic data

use cryptix_rpc_core::{api::rpc::RpcApi, GetBlockDagInfoResponse, GetServerInfoResponse};
use cryptix_wrpc_client::{
    client::{ConnectOptions, ConnectStrategy},
    prelude::NetworkId,
    prelude::NetworkType,
    result::Result,
    CryptixRpcClient, Resolver, WrpcEncoding,
};
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Cryptix node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // If you want to connect to your own node, define your node address and wRPC port using let url = Some("ws://0.0.0.0:19301")
    // Verify your Cryptix node is runnning with --rpclisten-borsh=0.0.0.0:19301 parameter
    ...
    // Define the network your Cryptix node is connected to
    ...
    let client = CryptixRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected Cryptix node
    client.connect(Some(options)).await?;

    // Retrieve and show Cryptix node information
    ...
    // Retrieve and show Cryptix network information
    ...
    // Disconnect client from Cryptix node
