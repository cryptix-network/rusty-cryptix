...
use cryptix_wrpc_server::{
    connection::Connection,
    router::Router,
    server::Server,
    service::{CryptixRpcHandler, Options},
};
use result::Result;
use std::sync::Arc;
use workflow_log::*;
...
struct Args {
    ...
    /// proxy:port for gRPC server (grpc://127.0.0.1:19201)
    #[clap(name = "grpc")]
    grpc_proxy_address: Option<String>,

    // /// wRPC port
    /// interface:port for wRPC server (wrpc://127.0.0.1:19301)
    #[clap(long)]
...
async fn main() -> Result<()> {
    ...
    let cryptixd_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{cryptixd_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(CryptixRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Cryptix wRPC server is listening on {}", options.listen_address);
