...
use cryptix_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId, utxo::utxo_inquirer::UtxoInquirerError};
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] cryptix_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] cryptix_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] cryptix_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] cryptix_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] cryptix_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] cryptix_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] cryptix_txscript::script_class::Error),
