...
use cryptix_grpc_core::{
    protowire::{
        ...
        CryptixdRequest, CryptixdResponse,
    },
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<CryptixdResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<CryptixdRequest>>,
