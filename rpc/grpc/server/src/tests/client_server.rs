...
use cryptix_notify::scope::{NewBlockTemplateScope, Scope};
use cryptix_rpc_core::api::rpc::RpcApi;
use cryptix_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    cryptix_core::log::try_init_logger("info, cryptix_grpc_core=trace, cryptix_grpc_server=trace, cryptix_grpc_client=trace");
