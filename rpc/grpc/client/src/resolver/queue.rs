use crate::{
    error::{Error, Result},
    resolver::{matcher::Matcher, CryptixdResponseReceiver, CryptixdResponseSender, Resolver},
};
...
struct Pending {
    ...
    sender: CryptixdResponseSender,
}

impl Pending {
    fn new(op: CryptixdPayloadOps, request: CryptixdRequest, sender: CryptixdResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &CryptixdResponse, response_op: CryptixdPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: CryptixdPayloadOps, request: &CryptixdRequest) -> CryptixdResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<CryptixdResponse>>();
        {
    ...
    fn handle_response(&self, response: CryptixdResponse) {
        let response_op: CryptixdPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
