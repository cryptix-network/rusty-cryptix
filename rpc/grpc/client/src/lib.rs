...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(CryptixdPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type CryptixdRequestSender = async_channel::Sender<CryptixdRequest>;
type CryptixdRequestReceiver = async_channel::Receiver<CryptixdRequest>;
...
/// Data flow:
/// ```
/// //   CryptixdRequest -> request_send -> stream -> CryptixdResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: CryptixdRequestSender,
    request_receiver: CryptixdRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<CryptixdResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual CryptixdRequest to CryptixdResponse stream
        let mut stream: Streaming<CryptixdResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: CryptixdPayloadOps, request: impl Into<CryptixdRequest>) -> Result<CryptixdResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: CryptixdRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<CryptixdResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = cryptixd_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = cryptixd_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
