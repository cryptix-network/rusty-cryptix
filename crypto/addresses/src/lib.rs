...
/// Address prefix identifying the network type this address belongs to (such as `cryptix`, `cryptixtest`, `cryptixsim`, `cryptixdev`).
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[borsh(use_discriminant = true)]
pub enum Prefix {
    #[serde(rename = "cryptix")]
    Mainnet,
    #[serde(rename = "cryptixtest")]
    Testnet,
    #[serde(rename = "cryptixsim")]
    Simnet,
    #[serde(rename = "cryptixdev")]
    Devnet,
    #[cfg(test)]
    A,
    #[cfg(test)]
...
impl Prefix {
    fn as_str(&self) -> &'static str {
        match self {
            Prefix::Mainnet => "cryptix",
            Prefix::Testnet => "cryptixtest",
            Prefix::Simnet => "cryptixsim",
            Prefix::Devnet => "cryptixdev",
            #[cfg(test)]
            Prefix::A => "a",
            #[cfg(test)]
            Prefix::B => "b",
...
impl TryFrom<&str> for Prefix {
    ...
    fn try_from(prefix: &str) -> Result<Self, Self::Error> {
        match prefix {
            "cryptix" => Ok(Prefix::Mainnet),
            "cryptixtest" => Ok(Prefix::Testnet),
            "cryptixsim" => Ok(Prefix::Simnet),
            "cryptixdev" => Ok(Prefix::Devnet),
            #[cfg(test)]
            "a" => Ok(Prefix::A),
            #[cfg(test)]
            "b" => Ok(Prefix::B),
...
mod tests {
    ...
    fn test_errors() {
        // cspell:disable
        let address_str: String = "cryptix:qqqqqqqqqqqqq1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        ...
        let address_str: String = format!("cryptix:qqqqqqqqqqqqq{invalid_char}qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e");
        let address: Result<Address, AddressError> = address_str.try_into();
        ...
        let address_str: String = "cryptix1:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::InvalidPrefix("cryptix1".into())), address);

        let address_str: String = "cryptixqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::MissingPrefix), address);

        let address_str: String = "cryptix:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4l".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);

        let address_str: String = "cryptix:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);
        // cspell:enable
    }
    ...
    pub fn test_wasm_serde_constructor() {
        let str = "cryptix:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j";
    ...
    pub fn test_wasm_js_serde_object() {
        let expected = Address::constructor("cryptix:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j");
        ...
        obj.set("prefix", &JsValue::from_str("cryptix")).unwrap();
