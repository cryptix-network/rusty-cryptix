...
use cryptix_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use cryptix_database::prelude::{DirectWriter, MemoryWriter};
use cryptix_database::prelude::{ReadLock, StoreError};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; cryptix_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use cryptix_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(cryptix_database::prelude::ConnBuilder::default().with_files_limit(10));
