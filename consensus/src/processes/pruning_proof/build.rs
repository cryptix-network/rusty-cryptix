...
use cryptix_database::prelude::{CachePolicy, ConnBuilder, StoreError, StoreResult, StoreResultEmptyTuple, StoreResultExtensions, DB};
...
impl<T: RelationsStoreReader, U: ReachabilityService> RelationsStoreReader for RelationsStoreInFutureOfRoot<T, U> {
    fn get_parents(&self, hash: Hash) -> Result<BlockHashes, cryptix_database::prelude::StoreError> {
    ...
    fn get_children(&self, hash: Hash) -> StoreResult<cryptix_database::prelude::ReadLock<BlockHashSet>> {
    ...
    fn counts(&self) -> Result<(usize, usize), cryptix_database::prelude::StoreError> {
        unimplemented!()
    }
}
...
impl PruningProofManager {
    pub(crate) fn build_pruning_point_proof(&self, pp: Hash) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = cryptix_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
