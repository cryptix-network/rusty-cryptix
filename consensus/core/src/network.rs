...
impl NetworkType {
    pub fn default_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 19201,
            NetworkType::Testnet => 19202,
            NetworkType::Simnet => 19203,
            NetworkType::Devnet => 19204,
        }
    }

    pub fn default_borsh_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 19301,
            NetworkType::Testnet => 19302,
            NetworkType::Simnet => 19303,
            NetworkType::Devnet => 19304,
        }
    }

    pub fn default_json_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 19401,
            NetworkType::Testnet => 19402,
            NetworkType::Simnet => 19403,
            NetworkType::Devnet => 19404,
        }
    }
...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'cryptix'.")]
...
impl NetworkId {
    ...
    pub fn default_p2p_port(&self) -> u16 {
        ...
        match self.network_type {
            NetworkType::Mainnet => 19101,
            NetworkType::Testnet => match self.suffix {
                Some(10) => 19102,
                Some(11) => 19103,
                None | Some(_) => 19104,
            },
            NetworkType::Simnet => 19111,
            NetworkType::Devnet => 19121,
        }
    }
    ...
    /// Returns a textual description of the network prefixed with `cryptix-`
    pub fn to_prefixed(&self) -> String {
        format!("cryptix-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("cryptix-") {
