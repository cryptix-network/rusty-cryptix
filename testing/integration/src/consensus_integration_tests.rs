...
use cryptix_consensus::config::genesis::GENESIS;
use cryptix_consensus::config::{Config, ConfigBuilder};
use cryptix_consensus::consensus::factory::Factory as ConsensusFactory;
use cryptix_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
use cryptix_consensus::model::stores::block_transactions::{
    BlockTransactionsStore, BlockTransactionsStoreReader, DbBlockTransactionsStore,
};
use cryptix_consensus::model::stores::ghostdag::{GhostdagStoreReader, KType as GhostdagKType};
use cryptix_consensus::model::stores::headers::HeaderStoreReader;
use cryptix_consensus::model::stores::reachability::DbReachabilityStore;
use cryptix_consensus::model::stores::relations::DbRelationsStore;
use cryptix_consensus::model::stores::selected_chain::SelectedChainStoreReader;
use cryptix_consensus::params::{
    ForkActivation, Params, DEVNET_PARAMS, MAINNET_PARAMS, MAX_DIFFICULTY_TARGET, MAX_DIFFICULTY_TARGET_AS_F64,
};
use cryptix_consensus::pipeline::monitor::ConsensusMonitor;
use cryptix_consensus::pipeline::ProcessingCounters;
use cryptix_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use cryptix_consensus::processes::window::{WindowManager, WindowType};
use cryptix_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use cryptix_consensus_core::block::Block;
use cryptix_consensus_core::blockhash::new_unique;
use cryptix_consensus_core::blockstatus::BlockStatus;
use cryptix_consensus_core::coinbase::MinerData;
use cryptix_consensus_core::constants::{BLOCK_VERSION, SOMPI_PER_CRYPTIX, STORAGE_MASS_PARAMETER};
use cryptix_consensus_core::errors::block::{BlockProcessResult, RuleError};
use cryptix_consensus_core::header::Header;
use cryptix_consensus_core::network::{NetworkId, NetworkType::Mainnet};
use cryptix_consensus_core::subnets::SubnetworkId;
use cryptix_consensus_core::trusted::{ExternalGhostdagData, TrustedBlock};
...
use cryptix_consensus_core::{blockhash, hashing, BlockHashMap, BlueWorkType};
use cryptix_consensus_notify::root::ConsensusNotificationRoot;
use cryptix_consensus_notify::service::NotifyService;
use cryptix_consensusmanager::ConsensusManager;
use cryptix_core::task::tick::TickService;
...
use cryptix_consensus_core::hashing::sighash::calc_schnorr_signature_hash;
use cryptix_consensus_core::merkle::calc_hash_merkle_root;
use cryptix_consensus_core::muhash::MuHashExtensions;
...
use cryptix_database::prelude::{CachePolicy, ConnBuilder};
...
use cryptix_txscript::opcodes::codes::OpTrue;
use cryptix_txscript::script_builder::ScriptBuilderResult;
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_cryptix_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = cryptix_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
...
async fn run_kip10_activation_test() {
    ...
    use cryptix_txscript::opcodes::codes::{Op0, OpTxInputSpk};
    use cryptix_txscript::pay_to_script_hash_script;
    ...
    let initial_utxo_collection = [(
        ...
        UtxoEntry { amount: SOMPI_PER_CRYPTIX, script_public_key: spk.clone(), block_daa_score: 0, is_coinbase: false },
    )];
...
async fn runtime_sig_op_counting_test() {
    ...
    use cryptix_txscript::{opcodes::codes::*, script_builder::ScriptBuilder};
    ...
    let script_pub_key = cryptix_txscript::pay_to_script_hash_script(&redeem_script);

    // Set up initial UTXO with P2SH script
    let initial_utxo_collection = [(
        TransactionOutpoint::new(1.into(), 0),
        UtxoEntry { amount: SOMPI_PER_CRYPTIX, script_public_key: script_pub_key.clone(), block_daa_score: 0, is_coinbase: false },
    )];
