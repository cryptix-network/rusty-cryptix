...
use cryptix_core::{info, task::tick::TickService, trace};
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), cryptix_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    cryptix_core::log::try_init_logger(
        "INFO, cryptix_core::time=debug, cryptix_rpc_core=debug, cryptix_grpc_client=debug, cryptix_notify=info, cryptix_notify::address::tracker=debug, cryptix_notify::listener=debug, cryptix_notify::subscription::single=debug, cryptix_mining::monitor=debug, cryptix_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    cryptix_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    cryptix_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    cryptix_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=19201 --p2p=19121 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    cryptix_core::log::try_init_logger(
        "INFO, cryptix_core::core=trace, cryptix_core::time=debug, cryptix_rpc_core=debug, cryptix_grpc_client=debug, cryptix_notify=info, cryptix_notify::address::tracker=debug, cryptix_notify::listener=debug, cryptix_notify::subscription::single=debug, cryptix_mining::monitor=debug, cryptix_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    cryptix_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), cryptix_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
...
/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package cryptix-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
