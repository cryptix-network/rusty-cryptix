...
use cryptix_core::{task::runtime::AsyncRuntime, trace};
use cryptix_grpc_client::GrpcClient;
use cryptix_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use cryptix_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  cryptix_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut cryptixd1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = cryptixd1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut cryptixd2 = Daemon::new_random(total_fd_limit);
...
async fn daemon_mining_test() {
    ...
    let mut cryptixd1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut cryptixd2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = cryptixd1.start().await;
    let rpc_client2 = cryptixd2.start().await;
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(cryptixd1.network.into(), cryptix_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            cryptix_consensus::params::SIMNET_GENESIS.hash, //
            true,
        )
        .await
        .unwrap();
...
/// `cargo test --release --package cryptix-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("cryptix-testing-integration-heap.json").build();

    cryptix_core::log::try_init_logger(
        "INFO,cryptix_testing_integration=trace,cryptix_notify=debug,cryptix_rpc_core=debug,cryptix_grpc_client=debug",
    );
    ...
    // Connect cryptixd2 to cryptixd1
    rpc_client2.add_peer(format!("127.0.0.1:{}", cryptixd1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    let miner_address =
        Address::new(cryptixd1.network.into(), cryptix_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(cryptixd1.network.into(), cryptix_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(cryptixd1.network.into(), cryptix_addresses::Version::PubKey, &[0; 32]);
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(cryptix_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&cryptixd2).await, ListeningClient::connect(&cryptixd1).await];
...
async fn daemon_cleaning_test() {
    ...
    {
        let total_fd_limit = 10;
        let mut cryptixd1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = cryptixd1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = cryptixd1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&cryptixd1.core);
