...
use cryptix_notify::{events::EventType, scope::Scope, subscription::Command};
use cryptix_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(cryptixd: &Daemon) -> Self {
        let client = cryptixd.new_multi_listener_client().await;
