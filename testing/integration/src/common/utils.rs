        ...
        utxoset
            ...
            .map(|(inputs, entries)| {
                ...
                let outputs = (0..num_outputs)
                    .map(|_| TransactionOutput { value: total_out / num_outputs, script_public_key: spk.clone(), payload: vec![] })
                    .collect_vec();
...
pub fn generate_tx(
    ...
    let outputs = (0..num_outputs)
        .map(|_| TransactionOutput { value: amount / num_outputs, script_public_key: script_public_key.clone(), payload: vec![] })
        .collect_vec();
    let unsigned_tx = Transaction::new(TX_VERSION, inputs, outputs, 0, SUBNETWORK_ID_NATIVE, 0, vec![]);
    let signed_tx =
