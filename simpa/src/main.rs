...
use cryptix_database::{create_temp_db, load_existing_db};
...
fn main() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "semaphore-trace")] {
            cryptix_core::log::init_logger(None, &format!("{},{}=debug", args.log_level, cryptix_utils::sync::semaphore_module_path()));
        } else {
            cryptix_core::log::init_logger(None, &args.log_level);
        }
    };
...
fn main_impl(mut args: Args) {
    ...
    let stop_perf_monitor = args.perf_metrics.then(|| {
        ...
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", cryptix_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", cryptix_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = CryptixNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
...
mod tests {
    ...
    fn test_pruning_via_simpa() {
        ...
        cryptix_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        cryptix_core::panic::configure_panic();
        main_impl(args);
    }
