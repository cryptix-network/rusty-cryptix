...
pub struct CryptixdHandshake<'a> {
    router: &'a Router,
...
impl<'a> CryptixdHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![CryptixdMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![CryptixdMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![CryptixdMessagePayloadType::Ready]),
        }
    }
