...
impl ConnectionHandler {
    ...
    fn outgoing_network_channel_size() -> usize {
        // TODO: this number is taken from go-cryptixd and should be re-evaluated
        (1 << 17) + 256
    }
...
impl ProtoP2p for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn futures::Stream<Item = Result<CryptixdMessage, TonicStatus>> + Send + 'static>>;

    /// Handle the new arriving **server** connections
    async fn message_stream(
        &self,
        request: Request<Streaming<CryptixdMessage>>,
