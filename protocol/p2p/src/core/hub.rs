use crate::{common::ProtocolError, pb::CryptixdMessage, ConnectionInitializer, Peer, Router};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: CryptixdMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: CryptixdMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: CryptixdMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<CryptixdMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();
