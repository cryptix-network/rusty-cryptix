...
impl Default for CryptixdConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<CryptixdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: CryptixdConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no cryptixd path is specified".to_string()));
        }
...
impl Cryptixd {
    pub fn new(args: CryptixdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: CryptixdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Cryptix node is already running.".to_string()));
            }
        }
        ...
        // let options = CryptixdOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Cryptixd binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait CryptixdCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: CryptixdConfig) -> Result<()>;
