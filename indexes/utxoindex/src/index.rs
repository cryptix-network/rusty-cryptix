...
use cryptix_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use cryptix_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use cryptix_core::{info, trace};
use cryptix_database::prelude::{StoreError, StoreResult, DB};
use cryptix_hashes::Hash;
use cryptix_index_core::indexed_utxos::BalanceByScriptPublicKey;
use cryptix_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-cryptixd), for quicker resets, while still having a low memory footprint.

/// UtxoIndex indexes `CompactUtxoEntryCollections` by [`ScriptPublicKey`](cryptix_consensus_core::tx::ScriptPublicKey),
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<cryptix_consensus_core::tx::TransactionOutpoint>> {
