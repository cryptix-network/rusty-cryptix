use crate::imports::*;
use cryptix_consensus_core::tx::{TransactionInput, TransactionOutpoint};
use cryptix_wallet_core::storage::Binding;
use cryptix_wallet_core::storage::{TransactionData, TransactionKind, TransactionRecord};
use cryptix_wallet_core::wallet::WalletGuard;
            ...
            | TransactionData::Change { utxo_entries, aggregate_input_value, .. } => {
                ...
                if include_utxos {
                    for utxo_entry in utxo_entries {
                        ...
                        let amount = transaction_type.style_with_sign(sompi_to_cryptix_string(utxo_entry.amount).as_str(), history);
            ...
            | TransactionData::TransferOutgoing { fees, aggregate_input_value, transaction, payment_value, change_value, .. } => {
                if let Some(payment_value) = payment_value {
                    lines.push(format!(
                        ...
                        "",
                        style(sompi_to_cryptix_string(*payment_value)).red(),
                        style(sompi_to_cryptix_string(*aggregate_input_value)).blue(),
                        style(sompi_to_cryptix_string(*fees)).red(),
                        style(sompi_to_cryptix_string(*change_value)).green(),
