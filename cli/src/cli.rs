...
use cryptix_daemon::{DaemonEvent, DaemonKind, Daemons};
use cryptix_wallet_core::account::Account;
use cryptix_wallet_core::rpc::DynRpcApi;
use cryptix_wallet_core::storage::{IdT, PrvKeyDataInfo};
use cryptix_wrpc_client::{CryptixRpcClient, Resolver};
...
impl From<&CryptixCli> for Arc<Terminal> {
    fn from(ctx: &CryptixCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<CryptixCli> for CryptixCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for CryptixCli {
...
impl CryptixCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                cryptix_core::log::init_logger(None, "info");
            } else {
                cryptix_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let cryptix_cli = Arc::new(CryptixCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(cryptix_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                cryptix_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<CryptixRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("Cryptix NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Cryptix NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Cryptix node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Cryptix node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Cryptix node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Cryptix node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let cryptixd = self.daemons().try_cryptixd();
            ...
            if let Some(cryptixd) = cryptixd.as_ref() {
                cryptixd.mute(false).await?;
                cryptixd.stop().await?;
            }
...
impl Cli for CryptixCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "cryptix" {
                prompt.push(title);
            }
...
impl cli::Context for CryptixCli {
...
pub async fn cryptix_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    CryptixCli::init();
    ...
    let banner =
        banner.unwrap_or_else(|| format!("Cryptix Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
