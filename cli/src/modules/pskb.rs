...
use cryptix_wallet_core::account::pskb::finalize_pskt_one_or_more_sig_and_redeem_script;
use cryptix_wallet_pskt::{
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        match action.as_str() {
            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    ...
                    "unlock" => {
                        ...
                        // Get locked UTXO set.
                        let spend_utxos: Vec<cryptix_rpc_core::RpcUtxosByAddressesEntry> =
                            ctx.wallet().rpc_api().get_utxos_by_addresses(vec![script_p2sh.clone()]).await?;
                        let priority_fee_sompi = try_parse_optional_cryptix_as_sompi_i64(argv.first())?.unwrap_or(0) as u64;
            ...
            "parse" => {
                ...
                let pskb = Self::parse_input_pskb(argv.first().unwrap().as_str())?;
                tprintln!(ctx, "{}", pskb.display_format(ctx.wallet().network_id()?, sompi_to_cryptix_string_with_suffix));
