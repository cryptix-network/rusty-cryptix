use crate::imports::*;
use cryptix_daemon::{locate_binaries, CpuMinerConfig};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "cryptix-cpu-miner").await {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<CryptixCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<CryptixCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<CryptixCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = cryptix_daemon::locate_binaries(root.as_str(), "cryptix-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No cryptix-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select cryptix-cpu-miner binary", &binaries).await? {
