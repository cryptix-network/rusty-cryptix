...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<CryptixCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, cryptix_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<CryptixCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <cryptix_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <cryptix_address> <signature>",
                    "Verify the signature against the message and cryptix_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<CryptixCli>, cryptix_address: &str, message: &str) -> Result<()> {
        let cryptix_address = Address::try_from(cryptix_address)?;
        if cryptix_address.version != Version::PubKey {
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<CryptixCli>, cryptix_address: &str, signature: &str, message: &str) -> Result<()> {
        let cryptix_address = Address::try_from(cryptix_address)?;
        if cryptix_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&cryptix_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<CryptixCli>, cryptix_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&cryptix_address])?;
