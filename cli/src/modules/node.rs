use crate::imports::*;
use cryptix_daemon::CryptixdConfig;
...
#[async_trait]
impl DefaultSettings for CryptixdSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        let mut settings = vec![(Self::Mute, to_value(true).unwrap())];

        let root = nw_sys::app::folder();
        if let Ok(binaries) = cryptix_daemon::locate_binaries(&root, "cryptixd").await {
...
pub struct Node {
    settings: SettingsStore<CryptixdSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("cryptixd").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<CryptixCli>() {
            ctx.daemons().clone().cryptixd.as_ref().map(|_| "node")
        } else {
            None
        }
    }
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Cryptix node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(CryptixdSettings::Mute) {
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<CryptixCli>) -> Result<CryptixdConfig> {
        let location: String = self
            .settings
            .get(CryptixdSettings::Location)
        ...
        let mute = false;
        let config = CryptixdConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<CryptixCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let cryptixd = ctx.daemons().cryptixd();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting cryptix node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting cryptix node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                cryptixd.configure(self.create_config(&ctx).await?).await?;
                cryptixd.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // cryptixd.mute(mute).await?;
                self.settings.set(CryptixdSettings::Mute, mute).await?;
            }
            "status" => {
                let status = cryptixd.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                cryptixd.configure(self.create_config(&ctx).await?).await?;
                let version = cryptixd.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<CryptixCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Cryptixd executable (binary) location"),
                ("version", "Display Cryptixd executable version"),
                ("start", "Start the local Cryptix node instance"),
                ("stop", "Stop the local Cryptix node instance"),
                ("restart", "Restart the local Cryptix node instance"),
                ("kill", "Kill the local Cryptix node instance"),
                ("status", "Get the status of the local Cryptix node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<CryptixCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = cryptix_daemon::locate_binaries(root.as_str(), "cryptixd").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No cryptixd binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a cryptixd binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(CryptixdSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(CryptixdSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/cryptixd'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<CryptixCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Cryptixd has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Cryptixd error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
