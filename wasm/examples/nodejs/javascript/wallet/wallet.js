...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToCryptixString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        list.push({
                            ...
                            Mature: sompiToCryptixString(data.balance.mature),
                            Pending: sompiToCryptixString(data.balance.pending),
                            Outgoing: sompiToCryptixString(data.balance.outgoing),
        ...
        // Send cryptix to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            accountId: firstAccount.accountId,
            priorityFeeSompi: cryptixToSompi("0.001"),
            destination:[{
                address: firstAccount.changeAddress,
                amount: cryptixToSompi("1.5")
            }]
        });
        console.log("sendResult", sendResult);

        // Transfer cryptix between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: cryptixToSompi("2.4"),
        });
