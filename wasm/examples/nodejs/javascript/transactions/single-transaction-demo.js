...
(async () => {
    ...
    // An address such as cryptix:qr0lr4ml9fn3chekrqmjdkergxl93l4wrk3dankcgvjq776s9wn9jkdskewva
    const address = keypair.toAddress(networkId);
    console.info(`Full cryptix address: ${address}`);
    ...
    try {
        ...
        if (utxos.length === 0) {
            console.info('Send some cryptix to', address, 'before proceeding with the demo');
            return;
        }
