...
| `--nodnsseed` | switch | `false` | Disable DNS peer seeding. This also disables Atomic bootstrap DNS seed sources. Normal P2P sync and local Atomic replay still work without RPC. On mainnet, optional configured-peer Atomic snapshot fallback additionally requires `--atomic-bootstrap-allow-peer-fallback` and enough independent configured bootstrap peers for quorum. The seed-confirmed non-seed minimum defaults to `2`, configured-peer fallback defaults to `3`, and both can be changed with `--atomic-bootstrap-peer-quorum-min-sources=<N>` for private/testing networks. |
