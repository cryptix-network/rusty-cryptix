...
//! the `cryptix` module includes `websocket` module dependency simulating
...
extern crate alloc;
extern crate self as cryptix_wallet_core;
...
pub fn version_with_git_hash() -> String {
    cryptix_utils::git::with_short_hash(env!("CARGO_PKG_VERSION")).to_string()
}
