...
use cryptix_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use cryptix_notify::{
    listener::ListenerId,
...
impl Wallet {
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<CryptixRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<CryptixRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<CryptixRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<CryptixRpcClient>().ok()).unwrap()
    }
    ...
    pub fn address_prefix(&self) -> Result<cryptix_addresses::Prefix> {
    ...
    pub async fn import_cryptixwallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(cryptix_bip32::Prefix::LENGTH).0;
        let prefix = cryptix_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_cryptixwallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_cryptixwallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(cryptix_bip32::Prefix::LENGTH).0;
        let prefix = cryptix_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_cryptixwallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(cryptix_bip32::Prefix::LENGTH).1.cmp(right.split_at(cryptix_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn scan_bip44_accounts(
        ...
        // TODO - this is not efficient, we need to scan without encrypting prv_key_data
        let prv_key_data =
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, cryptix_bip32::Error>>()?;
...
mod test {
    ...
    use cryptix_addresses::{Prefix, Version};
    use cryptix_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use cryptix_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use cryptix_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let address = Address::try_from("cryptixtest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("cryptixtest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(cryptix_bip32::AddressType::Receive))?;
