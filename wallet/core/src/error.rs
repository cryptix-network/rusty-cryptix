...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] cryptix_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    CryptixRpcClientResult(#[from] CryptixRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    CryptixWorkflowRpcError(#[from] CryptixWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] cryptix_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] cryptix_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] cryptix_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] cryptix_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] cryptix_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] cryptix_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] cryptix_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] cryptix_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] cryptix_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] cryptix_wallet_pskt::error::Error),
