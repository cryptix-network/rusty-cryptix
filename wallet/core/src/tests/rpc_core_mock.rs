...
use cryptix_notify::events::EVENT_TYPE_ARRAY;
use cryptix_notify::listener::{ListenerId, ListenerLifespan};
use cryptix_notify::notifier::{Notifier, Notify};
...
use cryptix_rpc_core::api::ctl::RpcCtl;
use cryptix_rpc_core::{api::connection::DynRpcConnection, api::rpc::RpcApi, *};
use cryptix_rpc_core::{notify::connection::ChannelConnection, RpcResult};
use std::sync::Arc;
...
impl RpcCoreMock {
    ...
    #[allow(dead_code)]
    pub fn notify_new_block_template(&self) -> cryptix_notify::error::Result<()> {
