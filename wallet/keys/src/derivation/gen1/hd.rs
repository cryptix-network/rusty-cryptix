...
pub struct WalletDerivationManager {
    /// extended public key derived upto `m/<Purpose>'/555555'/<Account Index>'`
...
impl WalletDerivationManager {
    ...
    fn create_extended_key(
        ...
        let address_path = format!("{purpose}'/555555'/{account_index}'");
    ...
    pub fn build_derivate_path(
        ...
        let purpose = if is_multisig { 45 } else { 44 };
        let mut path = format!("m/{purpose}'/555555'/{account_index}'");
    ...
    pub fn derive_child_pubkey_manager(
        ...
        let mut hmac = HmacSha512::new_from_slice(&public_key.attrs().chain_code).map_err(cryptix_bip32::Error::Hmac)?;
...
mod tests {
    ...
    async fn wallet_from_mnemonic() {
        ...
        let mnemonic = cryptix_bip32::Mnemonic::new(mnemonic, cryptix_bip32::Language::English).unwrap();
        let xprv = cryptix_bip32::ExtendedPrivateKey::<cryptix_bip32::SecretKey>::new(mnemonic.to_seed("")).unwrap();
        let xprv_str = xprv.to_string(cryptix_bip32::Prefix::KPRV).to_string();
        assert_eq!(
            xprv_str,
        ...
        let xpub_str = wallet.to_string(Some(cryptix_bip32::Prefix::KPUB)).to_string();
        assert_eq!(
            xpub_str,
    ...
    async fn address_test_by_ktrv() {
        ...
        let mnemonic = cryptix_bip32::Mnemonic::new(mnemonic, cryptix_bip32::Language::English).unwrap();
        let xprv = cryptix_bip32::ExtendedPrivateKey::<cryptix_bip32::SecretKey>::new(mnemonic.to_seed("")).unwrap();
        let ktrv_str = xprv.to_string(cryptix_bip32::Prefix::KTRV).to_string();
        assert_eq!(
            ktrv_str,
        ...
        let ktub_str = wallet.to_string(Some(cryptix_bip32::Prefix::KTUB)).to_string();
        assert_eq!(
            ktub_str,
