...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/cryptixnet/cryptixd/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        {
            let script_public_key = cryptix_txscript::pay_to_address_script(&cryptix_addresses::Address::new(
                prefix,
                cryptix_addresses::Version::PubKey,
                &[0u8; 32],
            ));
            let miner_data: MinerData = MinerData::new(script_public_key, vec![]);

            let BlockTemplate { block: cryptix_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use cryptix_consensus_core::subnets;
    use std::iter::repeat;
